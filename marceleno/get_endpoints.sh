...
test_subdomains(){
    ...
    local httpx_output=$(echo "$subdomains" | httpx -fc 200 -silent)
...
katana_endpoints(){
    ...
    local katana_output=$(echo "$subdomains" | katana -silent)"$katana_output"
...
main() {
    ...
    print_status "INFO" "Getting subdomains for $domain..."

    local subdomains=$(get_subdomains "$1")

    if [ -z "$subdomains" ]; then
        print_status "ERROR" "No subdomains found for $domain"
    else
        print_status "SUCCESS" "Found $(echo "$subdomains" | wc -l) subdomains!"
    fi

    print_status "INFO" "Testing subdomains..."
    local httpx_subdomains=$(test_subdomains "$subdomains")

    print_status "SUCCESS" "Tested: $(echo "$subdomains" | wc -l), Working: $(echo "$httpx_subdomains" | wc -l)"

    print_status "INFO" "Getting endpoints from $(echo "$httpx_subdomains" | wc -l) subdomains..."
    print_status "WARNING" "This may take a while!"

    print_status "INFO" "katana started!"
    local katana_output=$(katana_endpoints "$httpx_subdomains")
    print_status "SUCCESS" "Katana finished!"

    print_status "INFO" "waybackurls started!"
    local waybackurls_output=$(waybackurls_endpoints "$httpx_subdomains")
    print_status "SUCCESS" "waybackurls finished!"

    local endpoints_raw="$katana_output$waybackurls_output"
    ...
    print_status "SUCCESS" "Results saved at $domain/"
}
