...
def setPaths(rootPath):
    ...
    # custom extended User-Agent list (Aya custom)
    paths.CUSTOM_AGENTS = os.path.join(paths.SQLMAP_TXT_PATH, "custom-agents.txt")
...
def fetchRandomAgent():
    ...
    Depending on user options it can use either the default
    `user-agents.txt` list or the extended custom list
    `custom-agents.txt` (when --Aegnt is used).
    """
    ...
    if not kb.userAgents:
        # Choose which agents file to use
        agentsFile = paths.USER_AGENTS

        # Aya custom flag to use extended agents list
        if getattr(conf, "aegnt", False) and hasattr(paths, "CUSTOM_AGENTS"):
            agentsFile = paths.CUSTOM_AGENTS

        debugMsg = "loading random HTTP User-Agent header(s) from "
        debugMsg += "file '%s'" % agentsFile
        logger.debug(debugMsg)

        try:
            kb.userAgents = getFileItems(agentsFile)
        except IOError:
            errMsg = "unable to read HTTP User-Agent header "
            errMsg += "file '%s'" % agentsFile
            raise SqlmapSystemException(errMsg)
