...
def loadPayloads():
    ...
    # Aya custom:
    # If --AM is not used, skip am_payloads.xml even if present in PAYLOAD_XML_FILES
    payloadFiles = list(PAYLOAD_XML_FILES)

    if not conf.get("amPayloads"):
        payloadFiles = [payloadFile for payloadFile in payloadFiles if payloadFile != "am_payloads.xml"]
